...
let mobileSelectedItemId=null;
const layoutLegendKey=i=>i.environment&&!climateNames.includes(i.name)?`${i.label}|${i.name}|${i.environment}`:i.name;
const layoutHtml=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
function layoutSelectionText(item,items,ctx){
 if(!item)return null;
 const env=envFor(item,items,ctx),choice=climateNames.includes(item.name)?ctx.plan?.climate?.find(x=>x.building===item.name):null;
 const rawIndex=String(item.id||'').startsWith(`${item.name}:`)?Number(String(item.id).split(':').pop())+1:null,unit=Number.isFinite(rawIndex)?` #${rawIndex}`:'';
 const title=item.label&&item.label!==item.name?`${item.label} · ${item.name}${unit}`:`${item.name}${unit}`;
 const detail=[`Lv.${item.level||1}`,`${item.w}×${item.h} squares`];
 if(choice)detail.push(`${choice.mode} · 9×9 range`);
 else if(env.need)detail.push(env.need==='Adequate'?`Needs Adequate light · ${env.ok?'covered':'NOT covered'}`:`Needs ${env.need} temperature · ${env.ok?'covered':'NOT covered'}`);
 if(item.name==='Egg Incubator')detail.push('1-square access strip');
 return{title,detail:detail.join(' · '),legendKey:layoutLegendKey(item)};
}
function applyLayoutSelection(ctx,state,root,id){
 const item=id?state.items?.[id]:null;
 mobileSelectedItemId=item?id:null;
 const info=root?.querySelector('.layout-selection-info'),selected=item?layoutSelectionText(item,Object.values(state.items||{}),ctx):null;
 root?.querySelectorAll('.placed').forEach(el=>el.classList.toggle('layout-selected',!!item&&el.dataset.id===item.id));
 root?.querySelectorAll('[data-layout-legend-key]').forEach(el=>el.classList.toggle('layout-selected',!!selected&&decodeURIComponent(el.dataset.layoutLegendKey||'')===selected.legendKey));
 if(info){
  if(!selected){info.hidden=true;info.innerHTML=''}
  else{info.hidden=false;info.innerHTML=`<span>SELECTED</span><b>${layoutHtml(selected.title)}</b><small>${layoutHtml(selected.detail)}</small>`}
 }
}
...
function layoutPlanSignature(ctx,state){return JSON.stringify(desired(ctx,state).map(x=>({id:x.id,name:x.name,label:x.label,level:x.level||1,environment:x.environment||'',w:x.w,h:x.h})).sort((a,b)=>a.id.localeCompare(b.id)))}
function layoutPlanMismatch(ctx,state){const placed=Object.values(state.items||{});if(!placed.length)return false;const wanted=desired(ctx,state),wantMap=new Map(wanted.map(x=>[x.id,x]));if(placed.length!==wanted.length)return true;for(const p of placed){const w=wantMap.get(p.id);if(!w||p.name!==w.name||p.label!==w.label||Number(p.level||1)!==Number(w.level||1)||(p.environment||'')!==(w.environment||''))return true}return state.planSignature?state.planSignature!==layoutPlanSignature(ctx,state):false}
...
function draw(ctx,state,status=''){
 ...
 syncLayoutMismatchWarning(ctx,state,root);
 const limit=Math.min(ctx.level,16),unlocked=(state.unlocked||[]).filter(id=>+id<=limit),items=Object.values(state.items||{});if(mobileSelectedItemId&&!state.items?.[mobileSelectedItemId])mobileSelectedItemId=null;
 ...
 canvas.innerHTML=MAP_ORDER.map(id=>{const[c,r]=PLOTS[id],active=unlocked.includes(String(id)),available=id<=limit;return `<div class="land-plot ${active?'unlocked':''} ${available?'available':'locked'}" style="left:${c*PLOT_W*SCALE}px;top:${r*PLOT_H*SCALE}px;width:${PLOT_W*SCALE}px;height:${PLOT_H*SCALE}px"><span>Plot ${id}</span><small>${active?'OWNED':available?'AVAILABLE':`RV ${id}`}</small></div>`}).join('')+items.map(i=>{
  ...
  return `${climate?`<div class="climate-range ${i.name.replaceAll(' ','-').toLowerCase()}" style="left:${(i.x-3.5)*SCALE}px;top:${(i.y-3.5)*SCALE}px;width:${9*SCALE}px;height:${9*SCALE}px"></div>`:''}<div class="placed ${climate?'climate-device':''} ${env.need&&!env.ok?'bad-env':''} ${state.iconVisual!==false?'icon-view':''} ${mobileSelectedItemId===i.id?'layout-selected':''}" data-id="${i.id}" title="${description}" aria-label="${description}" style="--block-color:${colorFor(i)};left:${i.x*SCALE}px;top:${i.y*SCALE}px;width:${i.w*SCALE}px;height:${i.h*SCALE}px">${state.iconVisual!==false&&facilityIconPath(i.name,i.level)?`<img class="placed-icon" src="${facilityIconPath(i.name,i.level)}" alt="">`:''}<span class="sr-only">${description}</span></div>${a?`<div class="access-strip" style="left:${a.x*SCALE}px;top:${a.y*SCALE}px;width:${a.w*SCALE}px;height:${a.h*SCALE}px"></div>`:''}`;
 }).join('');
 const entries=new Map();for(const i of items){const key=layoutLegendKey(i);const e=entries.get(key)||{label:i.label===i.name?i.name:`${i.label} · ${i.name}`,detail:`${i.w}×${i.h}${i.environment?` · ${i.environment}`:''}`,color:colorFor(i),count:0,level:i.level||1};e.count++;entries.set(key,e)}
 ...
 applyLayoutSelection(ctx,state,root,mobileSelectedItemId);
