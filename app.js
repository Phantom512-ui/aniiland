...
function snapshotPlan(plan){if(!plan)return null;const result=structuredClone(plan);delete result.upgradePlan;return{appVersion:APP_VERSION,savedAt:Date.now(),userSignature:userPlanSignature(),result}}
...
function restoreAcceptedPlan(){const saved=acceptedPlanSnapshot;if(!saved||saved.userSignature!==userPlanSignature()||Number(saved.result?.level)!==level)return false;last=structuredClone(saved.result);usingSavedPlan=saved.appVersion!==APP_VERSION;return true}
...
function sharedCraftAllocation(r,rows){const allocations=new Map(),summaries=[];for(const facility of ['Woodworking Bench','Chimney Kiln']){const rs=rows.filter(row=>row.shared&&row.facility===facility);if(!rs.length)continue;const cfg=r.settings?.facilities?.[facility]||{},levels=Array.isArray(cfg.levels)?cfg.levels.flatMap(g=>Array.from({length:Math.max(0,Math.floor(Number(g.count)||0))},()=>Math.max(1,Number(g.level)||1))):Array.from({length:Math.max(0,Math.floor(Number(cfg.count)||0))},()=>Math.max(1,Number(cfg.level)||1));const machines=levels.sort((a,b)=>a-b).map((level,idx)=>({idx:idx+1,level,load:0,rows:[]}));const sorted=[...rs].sort((a,b)=>(byId.get(a.id)?.facilityLevel||1)-(byId.get(b.id)?.facilityLevel||1));for(const row of sorted){const need=byId.get(row.id)?.facilityLevel||1;let pool=machines.filter(m=>m.level>=need);if(!pool.length)continue;let machine=pool.find(m=>!m.rows.length)||pool.sort((a,b)=>a.load-b.load||a.idx-b.idx)[0];machine.rows.push(row);machine.load+=row.machineHours}for(const m of machines.filter(x=>x.rows.length)){const short=facility==='Woodworking Bench'?'Bench':'Kiln',label=`${short} ${m.idx}`;for(const row of m.rows)allocations.set(row.id,{label,rotates:m.rows.length>1,busy:m.load});summaries.push({facility,label,rows:m.rows,busy:m.load,rotates:m.rows.length>1})}}return{allocations,summaries}}
