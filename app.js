...
document.querySelector('.version').textContent='1.5.2';
...
const UI_STORE='aniimo-homeland-planner-ui-v143';
const LEGACY_UI_STORES=['aniimo-homeland-planner-ui-v141','aniimo-homeland-planner-ui-v140','aniimo-homeland-planner-ui-v135','aniimo-homeland-planner-ui-v134','aniimo-homeland-planner-ui-v132'];
...
const persistUiState=state=>{try{localStorage.setItem(UI_STORE,JSON.stringify(state))}catch{}};
...
function applyStrategySelection(value,{recalc=true,remember=true}={}){const select=$('#strategy');if(!select)return;if(remember)rememberScroll();select.value=value;plannerStrategy=value;syncStrategyButtons();if(value==='upgrade'&&level<20){$('#hours').value='rv';plannerHours='rv';autoDurationFromUpgrade=true}else if(autoDurationFromUpgrade&&$('#hours').value==='rv'){$('#hours').value='24';plannerHours='24';autoDurationFromUpgrade=false}else{plannerHours=$('#hours').value}saveUiState();last=null;if(recalc&&tab==='plan')calculate()}
...
function climateDiagram(r){
 ...
 return `<div class="climate-visual-toggle"><label class="check"><input type="checkbox" id="climate-icon-toggle" ${climateIconView?'checked':''}><span></span>Use facility icons</label></div><div class="climate-grid">${r.climate.map((c,ci)=>{
...
function workerCards(r){
 ...
 const cards=[...grouped.values()].sort((a,b)=>rank(a.ability)-rank(b.ability)).map(({ability,count})=>{
  ...
  const jobLines=jobs.map(job=>`<div class="worker-job"><div class="worker-job-main"><span class="worker-job-count">${job.count}</span><span class="worker-job-role">Working in ${esc(job.role)}</span></div><div class="worker-job-meta"><div class="worker-job-personality">Rec Personality: ${personalityBadge(job.personality)}</div><div class="worker-job-trait">Trait ${traitLevelText(job.traitLevel)}</div>${job.families?.length?`<div class="worker-job-family">Needed: ${job.families.map(f=>`${esc(f)} Family Aniimo`).join(' / ')}</div>`:''}</div></div>`).join('');
 ...
 if(freeSlots>=1&&r.settings.worker!=='minimum'){
  ...
  const water=(data.workerSuggestions?.Water||[]).filter(name=>!hiddenAniimoRecommendations.has(name)).slice(0,3);
  const waterSlots=freeSlots>=3?2:1,flexSlots=Math.max(0,freeSlots-waterSlots);
  const supportJobs=[
   `<div class="worker-job"><div class="worker-job-main"><span class="worker-job-count">+1</span><span class="worker-job-role">Water support for crop timers</span></div><div class="worker-job-meta"><div class="worker-job-personality">Use the first free slot on a Water Aniimo so Farmland and Woodland can keep the watered grow-time assumption.</div></div></div>`,
   flexSlots?`<div class="worker-job"><div class="worker-job-main"><span class="worker-job-count">+${flexSlots}</span><span class="worker-job-role">Flexible Earth / Grass / Fire support</span></div><div class="worker-job-meta"><div class="worker-job-personality">After the first watering slot, use free Aniimo to smooth farm-heavy or smelting-heavy plans.</div></div></div>`:'',
   waterSlots>1?`<div class="worker-job"><div class="worker-job-main"><span class="worker-job-count">+1</span><span class="worker-job-role">Second Water support</span></div><div class="worker-job-meta"><div class="worker-job-personality">If another slot is free, add a second Water Aniimo before spending later slots on more spare coverage.</div></div></div>`:''
  ].filter(Boolean).join('');
  cards.push(`<article class="worker-card flex-support-card" style="--ability-color:#45a9ef"><div class="worker-ability"><b>${freeSlots}+ spare slot${freeSlots===1?'':'s'}</b><img src="assets/spare-slots-icon.png" alt="Spare slots"></div><div class="worker-jobs">${supportJobs}</div><div class="aniimo-options grouped">${water.length?`<span class="aniimo-group"><b>Water</b>${water.map(name=>`<span>${aniimoIcon(name)}<small>${esc(name)}</small></span>`).join('')}</span>`:''}${earth.length?`<span class="aniimo-group"><b>Earth</b>${earth.map(name=>`<span>${aniimoIcon(name)}<small>${esc(name)}</small></span>`).join('')}</span>`:''}${grass.length?`<span class="aniimo-group"><b>Grass</b>${grass.map(name=>`<span>${aniimoIcon(name)}<small>${esc(name)}</small></span>`).join('')}</span>`:''}${fire.length?`<span class="aniimo-group"><b>Fire</b>${fire.map(name=>`<span>${aniimoIcon(name)}<small>${esc(name)}</small></span>`).join('')}</span>`:''}</div><em>Free-slot priority: Water 1 → Flexible support → Water 2 → any remaining spare support.</em></article>`);
 }
...
function toggleEventRecipeUnlock(id){const meta=EVENT_RECIPE_UNLOCKS.find(x=>x.id===id);if(!meta)return;if(eventRecipeUnlocks.has(id)){eventRecipeUnlocks.delete(id);if(eventRecipePaidUnlocks.has(id)){eventCurrency+=meta.cost;eventRecipePaidUnlocks.delete(id)}saveUiState();last=null;cache.clear();renderEventCenter();if(tab==='plan')calculate();return}if(!spendEventCurrency(meta.cost))return;eventRecipeUnlocks.add(id);eventRecipePaidUnlocks.add(id);saveUiState();last=null;cache.clear();renderEventCenter();if(tab==='plan')calculate()}
...
resetConfig();syncStrategyButtons();saveUiState();renderOrderDock();await calculate();
